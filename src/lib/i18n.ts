...
const translations = {
  ...
  fr: {
    siteName: 'MTL Metro',
    siteDescription: 'Trouvez le trajet le plus rapide sur le réseau de metro, REM et trains de banlieue Exo de Montréal.',
    siteTagline: 'Planificateur de trajet Métro Montréal',
    ...
    heroSubtitle: 'Planifiez votre trajet sur le métro STM, le REM et les trains de banlieue Exo.',
    heroBadge: (count: number) => `${count} stations sur 3 réseaux`,
    networkTitle: 'Le réseau',
    networkSubtitle: 'Trois systèmes de transport interconnectés, un seul tarif.',
    stmLabel: 'Métro STM',
    remLabel: 'REM',
    exoLabel: 'Trains Exo',
    stationsAcrossLines: (count: number, lines: number) => `${count} stations, ${lines} lignes`,
    stationsAutomated: (count: number) => `${count} stations, train léger automatisé`,
    stationsTrainLines: (count: number, lines: number) => `${count} stations, ${lines} lignes de train`,
    fareZonesTitle: 'Zones tarifaires',
    fareZonesSubtitle: 'Tarification unifiée sur tous les modes. Valide 120 minutes.',
    farePayment: 'Carte OPUS ou paiement sans contact',
    planTripTitle: 'Planifiez votre trajet à Montréal',
    planTripText: 'MTL Metro calcule le trajet le plus rapide entre deux stations du réseau de transport de Montréal. Que vous voyagiez sur le métro STM, le REM ou les trains de banlieue Exo, obtenez le trajet optimal avec les correspondances, le temps de trajet, les arrêts et les tarifs.',
    montrealIsland: 'Île de Montréal',
    montrealLaval: 'Montreal + Laval',
    innerSuburbs: '+ Banlieue proche',
    fullNetwork: 'Réseau complet',
    ...
    fare: 'Tarif',
    stepByStepRoute: 'Trajet étape par étape',
    ...
    departure: 'Départ',
    arrival: 'Arrivée',
    transferAt: (station: string) => `Correspondance à ${station}`,
    walkMin: '~3 min à pied',
    ...
    faqTimeAnswer: (from: string, to: string, time: number, stops: number, transfers: number) =>
      `Le trajet de ${from} a ${to} prend environ ${time} minutes avec ${stops} arrêts${transfers > 0 ? ` et ${transfers} correspondance(s)` : ' (direct, sans correspondance)'}.`,
    faqCost: 'Combien coûte le tarif ?',
    faqCostAnswer: (price: string, type: string, validity: number) =>
      `Le tarif est de ${price} (${type}). Le billet est valide ${validity} minutes sur tous les modes de transport.`,
    faqTransfers: 'Combien de correspondances sont nécessaires ?',
    faqTransfersNone: (line: string) => `Aucune correspondance nécessaire. C'est un trajet direct sur la ${line}.`,
    ...
    leg: 'Troncon',
    planMultiStop: 'Planifiez votre trajet multi-arrêts sur le réseau de Montréal.',

    departAt: 'Départ à',
    arriveBy: 'Arriver avant',
    today: "Aujourd'hui",
    tomorrow: 'Demain',
    estimatedArrival: 'Arrivée est.',
    estimatedDeparture: 'Départ est.',
    ...
    accessibilityGuide: "Guide d'accessibilité",
    accessibilityDescription: "Informations sur l'accessibilité des stations du Metro, REM et Exo de Montréal.",
    fullyAccessible: 'Entièrement accessible',
    limitedAccessibility: 'Accessibilité limitée',
    accessibleStations: (count: number, total: number) => `${count} de ${total} stations sont entièrement accessibles`,
    accessibilityTips: "Conseils d'accessibilité",
    ...
    calculate: 'Calculer',
    selectBothStations: 'Selectionnez les deux stations pour voir la comparaison des coûts.',
    tripsPerMonth: 'trajets/mois',
    costCalculatorIntro: 'Entrez vos stations de départ et de travail pour comparer le paiement par trajet et la passe mensuelle. Decouvrez quelle option vous fait economiser le plus selon la frequence de vos deplacements.',

    // Share
    shareRoute: 'Partager le trajet',
    copied: 'Copie!',

    footerDescription: 'Planificateur de trajet couvrant le métro, le REM et les trains de banlieue Exo de Montréal.',
    navigate: 'Navigation',
    ...
    compareLinesDesc: 'Comparaison de toutes les lignes de transport de Montréal.',
